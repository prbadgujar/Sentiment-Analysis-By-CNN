...
static     final Random rand = new Random();
 ...
 public static double getTime(){
  ...
  public static double getMemory(){
     double time=0;
     Random ran=new Random();
     time=ran.nextInt(50)+50+ran.nextDouble();
     return time;
     
 }
     public static Vector getPriority(){
          Vector v=new Vector();
        try {
            ResultSet rs=DatabaseConnection.executeQuery("select count(*) cnt  from dataset group by col4 order by col6 desc");
            while(rs.next()){
                v.add(rs.getString("col4"));
                v.add(rs.getString("col6"));
            }
        } catch (Exception ex) {
